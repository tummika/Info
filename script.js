    ...
    if (p.pupilData) {
      ...
      chart = new Chart(canvas, {
          ...
          datasets: [
            {
              ...
              borderWidth: 1,
              pointRadius: 0,
              pointHitRadius: 6// 🟢 ทำให้ hover ง่ายขึ้นมาก
            },
            {
              label: 'Current Time',
              data: [{ x: 0, y: null }],
        ...
        options: {
          animation: false,
          interaction: {
            mode: 'index',       // 🟢 แสดง tooltip ตามแกน x เดียวกัน
            intersect: false     // 🟢 ไม่ต้อง hover ตรงจุดพอดี
          },
          ...
          plugins: {
            ...
            tooltip: {
              enabled: true,
              callbacks: {
                title: () => '', // ❌ ซ่อนหัว tooltip
                
                label: function (context) {
                  const datasetLabel = context.dataset.label;
                  
                  if (datasetLabel === "Current Time") {
                    return [`Time: ${xSec}s`, `Avg z-score pupil size: ${yVal}`]; // ✅ กรณีเส้นหลัก
                  }
